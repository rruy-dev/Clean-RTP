...
execute if entity @s[tag=begin_preload] at @s as @e[type=minecraft:armor_stand,scores={rtp_cooldown=99},tag=preloader] if score @s rtp_tag = @e[type=minecraft:armor_stand,tag=begin_preload,distance=..1,limit=1] rtp_tag run tag @e[type=minecraft:armor_stand,tag=begin_preload,distance=..1,limit=1] remove begin_preload
...
execute unless entity @s[tag=rtping] run return 0
tag @s remove rtping
