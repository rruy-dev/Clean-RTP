...
execute as @e[type=minecraft:armor_stand,scores={rtp_cooldown=1..}] run scoreboard players remove @s rtp_cooldown 1
...
execute unless entity @e[type=minecraft:armor_stand,scores={rtp_tag=0..},limit=1] as @a[scores={rtp_tag=0..}] run function clean_rtp:cancelrtp
execute unless entity @e[type=minecraft:armor_stand,scores={rtp_tag=0..},limit=1] run return 0
execute unless entity @a[scores={rtp_tag=0..}] run execute as @e[type=minecraft:armor_stand,scores={rtp_tag=0..},tag=preloader] at @s run return run function clean_rtp:cancelpreload
